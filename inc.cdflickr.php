<?php
/*
 * CdFlickr written by cidy0106 (cidy0106@gmail.com) 
 * 一开始想使用phpflickr，但一直出现出错，不知道是否是因为flickr开始使用了oauth1授权
 *  急于完成项目，而原项目又基于phpflickr，所以就在phpflickr的基础上（尽量不改变原有逻辑），
 *  进行以下修改：
 *   1、支持oauth1授权，并修改了api内部实现； 
 *   2、目前只用curl提交请求 
 *   3、支持使用代理服务器
 *   4、去掉session部分
 */
if (! class_exists ( 'CdFlickr' )) {
// 	if (session_id () == "") {
// 		@session_start ();
// 	}
	class CdFlickr {
		...
		var $token_secret = false; // 授权的secret
		var $proxy = false;
		var $proxy_port; // proxy
		
		function CdFlickr($api_key, $secret = NULL, $die_on_error = false) {
		...
		/**
		 * http proxy server and port,api will use it if it set
		 *
		 * @param unknown $proxy        	
		 * @param unknown $port        	
		 */
		function enableCache($type, $connection, $cache_expire = 600, $table = 'flickr_cache') {
			...
			if ($type == 'db') {
				if (preg_match ( '|mysql://([^:]*):([^@]*)@([^/]*)/(.*)|', $connection, $matches )) {
					...
					/*
					 * If high performance is crucial, you can easily comment out this query once you've created your database table.
					 */
					mysqli_query ( $db, "
...
expiration=TIMESTAMPADD(SECOND,' . $this->cache_expire . ',CURRENT_TIMESTAMP) ';
		...
		function post($url, $data = false) {
			...
			if (function_exists ( 'curl_init' )) {
				...
				$curl = curl_init ( $url );
				if (! empty ( $this->proxy )) {
					curl_setopt ( $curl, CURLOPT_PROXY, $this->proxy . ':' . $this->proxy_port );
					curl_setopt($curl, CURLOPT_HTTPPROXYTUNNEL, CURLPROXY_HTTP);//如果是HTTP代理
				}
				...
				die ( "Sorry, your server must support CURL in order to call flickr api" );
			}
			return $response;
		}
		/**
		 ...
		 *        	授权后的secret
		 */
		function post_before($method, $baseurl, &$args, $token_secret=false) {
			// 使用了oauth1，需要跟一些oauth1需要的参数
			// 并且需要计算signature by xidige 2014-9-19
			...
			if (! empty ( $token_secret )) {
				...
				if(empty($args['oauth_token'])){
					if( empty($this->token) ){
						die('must be set token and token_secret before the flickr api call');
					}else{
						$args['oauth_token']= $this->token;
					}
				}
				$signature = $this->get_signature ( $method, $baseurl, $args, $token_secret );
			} else {
				$signature = $this->get_signature ( $method, $baseurl, $args );
			}
			$args ['oauth_signature'] = $signature;
		}
		/**
		 * 用在普通api调用，需要token和token_secret，不支持文件上传下载
		 *
		...
		function request($command, $args = array(), $nocache = false) {
			...
			$this->post_before ( 'POST', $this->rest_endpoint, $args ,$this->token_secret );
			
			$this->last_request = $args;
			$this->response = $this->getCached ( $args );
			if (! ($this->response) || $nocache) {
				$this->response = $this->post ( sprintf ( '%s?%s', $this->rest_endpoint, http_build_query ( $args ) ) );
				$this->cache ( $args, $this->response );
			}
			/*
			 * Uncomment this line (and comment out the next one) if you're doing large queries and you're concerned about time. This will, however, change the structure of the result, so be sure that you look at the results.
			 */
...
// 					'photo' => '@' . realpath ( $photo ) 
		...
		/**
		 * 使用oauth1授权方式,第一步：获取临时token和secret
		 * 返回数组
		 * 
		 ...
		 */
		function oauth_requestcode($callbackurl) {
			$args = array (
					'oauth_callback' => $callbackurl 
			);
			$this->post_before ( 'POST', $this->requestTokenUrl, $args );
			// oauth_callback_confirmed=true
			// &oauth_token=72157626737672178-022bbd2f4c2f3432
			// &oauth_token_secret=fccb68c4e6103197
			$response = $this->post ( sprintf ( '%s?%s', $this->requestTokenUrl, http_build_query ( $args ) ) );
			
			return $this->parse_query ( $response );
		}
		/**
		 * 使用oauth1授权方式,第三步：获取accesstoken
		 *
		 * @param unknown $oauth_token
		 *        	临时token
		 * @param unknown $oauth_secret
		 *        	临时secret
		 * @param unknown $oauth_verifier
		 *        	授权成功后接受到的oauth_verifier值
		 */
		function oauth_accesstoken($oauth_token, $oauth_secret, $oauth_verifier) {
			$args = array (
					'oauth_token' => $oauth_token,
					'oauth_verifier' => $oauth_verifier 
			);
			$this->post_before ( 'POST', $this->accessTokenUrl, $args, $oauth_secret );
			
			$response = $this->post ( sprintf ( '%s?%s', $this->accessTokenUrl, http_build_query ( $args ) ) );
		 ...
		 * @param $method GET
		 *        	POST
		 * @param $baseurl 没有query的地址        	
		 * @param $params 数组        	
		 * @param $withsecret 是否增加secret作为key，记得设置$token_secret        	
		 */
		function get_signature($method, $baseurl, $params, $withsecret = false) {
			ksort ( $params );
			if (! empty ( $withsecret )) {
				$signing_key = $this->secret . '&' . $withsecret;
			} else {
			...
			foreach ( $params as $key => $value ) {
				array_push ( $output, urlencode ( $key ) . '=' . urlencode ( $value ) );
			}
			$query = implode ( '&', $output );
			
			$signing_data = $method . '&' . urlencode ( $baseurl ) . '&' . urlencode ( $query );
			return base64_encode ( hash_hmac ( 'sha1', $signing_data, $signing_key, true ) );
		}
		 ...
		 * to use and an associative array of arguments.
		 * For example:
		...
		/*
		 * These functions are the direct implementations of flickr calls. For method documentation, including arguments, visit the address included in a comment in the function.
		 */
/* Activity methods */
function activity_userComments($per_page = NULL, $page = NULL) {
		 ...
		 * @param unknown $frob
		 */
// 		function auth_getToken($frob=false){
// 			return $this->oauth_checkToken();
// 		}
// 		function auth_checkToken() {
// 			return $this->oauth_checkToken();
// 		}
// 		function auth_getFullToken($mini_token=false){
// 			return $this->oauth_checkToken();
// 		}
		/**
		 * *********old code***开始***********
...
// 		}
		...
		// $this->request ( 'flickr.auth.getFullToken', array (
...
// 		function auth_getToken($frob) {
			...
			// $this->request ( 'flickr.auth.getToken', array (
			// 'frob' => $frob
			// ) );
